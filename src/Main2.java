...
public class Main2 {
	...
	public static void main(String[] args) throws UnknownHostException, IOException {
		...
		System.out.println("Iniciou");
			...
			while(message == null) {
				serversocket = new ServerSocket(6782); // Qual porta eu uso e como decido qual porta?
			...
			askingsocket = new ServerSocket(6782);
			System.out.println("Aguardando respostas");
			while(respostas < 2) {//contatosNG.size()) {
				if(quemFala) {
				...
				try {
					...
					quemFala = Boolean.valueOf(resposta[1]);
